...
import { CODE_EXPIRY_SECONDS, COGNITO_OAUTH_CODE_URI, CLIENT_ID, REDIRECT_URI, TABLE_NAME} from './config.js'
...
async function handler (event, context) {
    ...
    //We would present a proceed/cancel page here, but for simplicity we'll just redirect
    const queryInput = {
    ...
    if (ddbQueryResponse.Items?.length === 0) {
        return htmlResponse(400, 'Invalid user code. Maybe it expired?');
    }
