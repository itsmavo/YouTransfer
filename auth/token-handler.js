...
async function handler(event, context) {
    ...
    logger.debug({getItemInput}, 'Reading device code');
    ...
    switch (status) {
        ...
        default:
            return jsonResponse(400, { error_code: OAuthErrorCodes.INVALID_GRANT,
...
async function fetchToken(oauthCode) {
    ...
    const { statusCode, statusText } = tokenResponse;
    const responseText = await tokenResponse.body.text();
    const response = {
        statusCode,
        statusText,
        body: responseText,
    ...
    logger.info({response}, 'Received token response');
    return response;
}
